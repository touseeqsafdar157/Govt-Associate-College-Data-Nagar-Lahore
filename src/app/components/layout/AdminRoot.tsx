...
import {
  ...
  Image, Bell, GraduationCap, ChevronRight, Home,
} from "lucide-react";
...
export function AdminRoot() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:sticky top-0 h-screen w-64 bg-[#003D1F] text-white flex flex-col z-50 transition-transform duration-300 ${
      ...
      >
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-5 py-3 text-sm transition-all ${
                  active
                    ? "bg-[#C8A951] text-[#003D1F] font-semibold"
                    : "text-gray-300 hover:bg-[#006B3F]/40 hover:text-white"
                }`}
              >
        ...
        <div className="p-4 border-t border-[#006B3F]/50 space-y-2">
          <Link
            to="/"
            className="flex items-center gap-2 text-sm text-gray-300 hover:text-[#C8A951] transition-colors px-1 py-1"
          >
            <Home className="w-4 h-4" /> View Website
          </Link>
          <button
            onClick={logout}
            className="flex items-center gap-2 text-sm text-red-400 hover:text-red-300 transition-colors px-1 py-1 w-full"
          >
