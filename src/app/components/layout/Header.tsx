...
import { Phone, Mail, Facebook, Youtube, Instagram, Menu, X, ChevronDown, ChevronRight } from "lucide-react";
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col sm:flex-row justify-between items-center text-xs gap-1.5">
            ...
            <div className="flex items-center gap-3">
              ...
              <a href="https://www.facebook.com/profile.php?id=61557940628161" target="_blank" rel="noopener noreferrer" className="w-6 h-6 bg-[#006B3F] hover:bg-[#C8A951] rounded-full flex items-center justify-center transition-colors">
                <Facebook className="w-3  h-3" />
              </a>
              <a href="https://www.instagram.com/principalgacwdatanagar?igsh=MThlYmY0M3d5NmF1Nw==" target="_blank" rel="noopener noreferrer" className="w-6 h-6 bg-[#006B3F] hover:bg-[#C8A951] rounded-full flex items-center justify-center transition-colors">
              ...
              return (
                ...
                >
                  {link.path ? (
                    <Link
                      to={link.path}
                      className={`flex items-center gap-1 px-4 py-3.5 text-sm text-white hover:bg-[#C8A951] hover:text-[#003D1F] transition-all duration-200 font-medium ${isActive ? "bg-[#C8A951] text-[#003D1F]" : ""
                        }`}
                    >
                      {link.label}
                    </Link>
                    ...
                    <div className={`absolute top-full left-0 w-48 bg-white shadow-xl border-t-2 border-[#C8A951] z-50 transition-all duration-200 ${openDropdown === link.label ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-1"}`}>
                      {link.children.map((child) => (
                        <Link
                          key={child.path}
                          to={child.path}
                          className={`flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#F0F7F4] hover:text-[#006B3F] transition-colors border-b border-gray-100 last:border-0 ${location.pathname === child.path ? "bg-[#F0F7F4] text-[#006B3F] font-semibold" : ""
                            }`}
                        >
          ...
          {navLinks.map((link) => (
            <div key={link.label}>
              {link.path ? (
                <Link
                  to={link.path}
                  className={`flex items-center px-5 py-3 text-white border-b border-[#006B3F]/40 text-sm hover:bg-[#C8A951] hover:text-[#003D1F] transition-colors ${location.pathname === link.path ? "bg-[#C8A951] text-[#003D1F]" : ""
                    }`}
                >
                  {link.label}
                </Link>
